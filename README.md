...
持久化保存报告、图片、音频、CSV 等文件，支持返回本地路径或 AstrBot 临时下载 URL。

| 工具 | 功能 |
|------|------|
| `nkit_file_save` | 保存或覆盖文件，可设置保留天数 |
| `nkit_file_get_path` | 获取已保存文件的本地绝对路径，用于 AstrBot 内部共享文件 |
| `nkit_file_get_url` | 获取已保存文件的临时下载 URL，访问一次后 URL 会被销毁 |
| `nkit_file_list` | 列出当前作用域下的文件 |
| `nkit_file_delete` | 删除已保存文件 |

文件保存在插件数据目录的 `file_store` 子目录中，隔离策略与 KV 存储共同使用 WebUI 的“存储隔离”配置。保存时可指定 `retention_days`，默认保留 7 天，`-1` 表示永久保留。`get_url` 使用 AstrBot 文件服务生成临时链接，适合发送给用户下载。
