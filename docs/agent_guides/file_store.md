...
4. **路径优先给工具用**：后续工具需要本地文件时使用 `nkit_file_get_path`，不要把临时 URL 当成本地路径。
...
| `nkit_file_get_path` | 获取本地绝对路径，用于 AstrBot 内部共享文件 | `key` |
...
返回本地绝对路径，用于 AstrBot 内部共享文件，适合传给图片识别、文件处理或其他插件内部逻辑。
